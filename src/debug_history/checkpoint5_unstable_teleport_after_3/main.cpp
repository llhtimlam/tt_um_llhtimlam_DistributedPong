...
// Convert 3-bit unsigned value to signed 4-bit (range -4..+3)
int to_signed_4bit(int val3) {
    if (val3 & 4) return val3 - 8;  // sign-extend
    return val3;
}
...
int main(int argc, char* argv[]) {
    ...
    int last_left_ball = 0, last_right_ball = 0;
    int left_ball_spawned = 0, right_ball_spawned = 0;
    
    // Increased spawn detection timeout (2 seconds at 25 MHz = 50M cycles)
    int right_spawn_check = 0;
    const int SPAWN_TIMEOUT_CYCLES = 50000000;
    ...
    while (!done && frame_count < 10000) {
        ...
        if (tx_phase == 1 && top_left->uio_oe) {
            ...
            int vel_x3 = (tx_byte2 >> 3) & 0x07;
            int vel_y3 = (tx_byte2 >> 0) & 0x07;
            printf("   TX byte2 = 0x%02X (Y high+vel)\n", tx_byte2);
            printf("   -> Y = %d, vx = %d, vy = %d\n",
                (tx_byte2 >> 6) * 256 + tx_byte1,
                to_signed_4bit(vel_x3),
                to_signed_4bit(vel_y3));
            tx_phase = 0;
        }
        ...
        if (rx_phase == 1 && top_right->uio_in != 0) {
            ...
            int vel_x3 = (rx_byte2 >> 3) & 0x07;
            int vel_y3 = (rx_byte2 >> 0) & 0x07;
            printf("   RX byte2 = 0x%02X\n", rx_byte2);
            printf("   -> Y = %d, vx = %d, vy = %d\n",
                (rx_byte2 >> 6) * 256 + rx_byte1,
                to_signed_4bit(vel_x3),
                to_signed_4bit(vel_y3));
            rx_phase = 0;
        }
        ...
        // Spawn detection
        int left_ball = top_left->uo_out & 1;
        int right_ball = top_right->uo_out & 1;
        
        if (right_ui_sync && !last_right_ui_sync) {
            right_spawn_check = SPAWN_TIMEOUT_CYCLES;
        }
        if (right_spawn_check > 0) {
            right_spawn_check--;
            if (right_ball && !last_right_ball) {
                printf("*** RIGHT BALL SPAWNED (cycle %d)\n", cycle_count);
                right_spawn_check = 0;
            } else if (right_spawn_check == 1 && !right_ball) {
                printf("*** Note: Right ball spawn took > %d cycles (still OK)\n", SPAWN_TIMEOUT_CYCLES);
            }
        }
        
        if (left_ball && !last_left_ball && !left_ball_spawned) {
            left_ball_spawned = 1;
            printf("*** LEFT BALL SPAWNED at cycle %d\n", cycle_count);
        }
        if (right_ball && !last_right_ball && !right_ball_spawned) {
            right_ball_spawned = 1;
            printf("*** RIGHT BALL SPAWNED at cycle %d\n", cycle_count);
        }
        if (!left_ball) left_ball_spawned = 0;
        if (!right_ball) right_ball_spawned = 0;
        
        last_left_ball = left_ball;
        last_right_ball = right_ball;
        ...
        if (cycle_count % 10000000 == 0) {
            ...
            printf("LEFT: uo_out=0x%02X (sync=%d, edge=%d, ball=%d) ui_in=0x%02X\n",
                top_left->uo_out, left_sync, left_edge, left_ball, top_left->ui_in);
            printf("RIGHT: uo_out=0x%02X (sync=%d, edge=%d, ball=%d) ui_in=0x%02X\n",
                top_right->uo_out, right_sync, right_edge, right_ball, top_right->ui_in);
        }
