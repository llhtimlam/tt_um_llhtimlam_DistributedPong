...
module tt_um_vga_example (
    ...
    // Edge detection (widened to match release)
    ...
    // ------------------------------------------------------------------
    //  Cross-domain handshake for spawn_ball
    // ------------------------------------------------------------------
    ...
    // clk_pix domain: set request on spawn_ball, clear on ack
    always @(posedge clk_pix) begin
    ...
    // vsync domain: sample request and spawn ball
    always @(posedge vsync or posedge reset) begin
        if (reset) begin
            ...
            // sample the request (metastability safe)
            spawn_req_sync <= spawn_req_meta;
            
            // acknowledge and spawn
            if (spawn_req_sync && !has_ball) begin
                spawn_ack <= 1;
                has_ball <= 1;
                ball_y <= pending_y;
                vel_x <= pending_phys[7:4];
                vel_y <= pending_phys[3:0];
...
module chip_comm_manager (
    ...
    // Receive shift register – samples bus while sync is high
    always @(posedge clk) begin
        if (reset) begin
            rx_shift_cnt <= 0;
            rx_shift_reg <= 0;
            ...
            // Reset on rising edge of sync
            if (in_sync_pin && !in_sync_prev) begin
                rx_shift_cnt <= 0;
                rx_shift_reg <= 0;
            end

            // While sync is high, shift in bytes
            if (in_sync_pin) begin
                if (rx_shift_cnt == 0) begin
                    ...
                    received_y <= {rx_shift_reg[15:14], rx_shift_reg[7:0]};
                    // verilator lint_on WIDTHTRUNC
                    received_physics <= {bus_in[5:3], 1'b0, bus_in[2:0], 1'b0};
                    spawn_ball <= 1;
                end
            end
        end
