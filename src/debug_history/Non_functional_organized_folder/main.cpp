...
void update_display(Display* d, int hpos, int vpos, bool de, uint8_t r, uint8_t g, uint8_t b) {
    if (de && hpos >= 0 && hpos < H_RES && vpos >= 0 && vpos < V_RES) {
...
int main(int argc, char* argv[]) {
    ...
    Vproject* left  = new Vproject;
    Vproject* right = new Vproject;
    ...
    vluint64_t sim_time = 0;    // Simulation time variable
    int cycle = 0;
    bool quit = false;
    // For UART console logging (optional)
    int last_left_tx = -1, last_right_tx = -1;
    ...
    while (!quit && cycle < 100000000) {
        ...
        int left_tx_bit = (left->uio_out >> 0) & 1;
        int left_rx_bit = (left->uio_in >> 1) & 1;
        //printf("time %lu: Left TX = %d\n", (unsigned long)sim_time, left_tx_bit);
        int right_rx_bit = (right->uio_in >> 1) & 1;
        if (cycle % 2604 == 1) {
        printf("Right RX = %d|%d|%d\n", left->dd, right_rx_bit, left->dc);
        }
        
        static int last_tx_valid = 0;
        if (left->dd && !last_tx_valid) {
            printf("rx%d, %d\n", left->dd, right_rx_bit);
        }
        last_tx_valid = left->dd;
        /*
        int left_tx_bit = (left->uio_out >> 0) & 1;
        if (left_tx_bit != last_left_tx) {
            printf("time %llu: Left TX = %d\n", sim_time, left_tx_bit);
            last_left_tx = left_tx_bit;
        }
        int right_tx_bit = (right->uio_out >> 0) & 1;
        if (right_tx_bit != last_right_tx) {
            printf("time %llu: Right TX = %d\n", sim_time, right_tx_bit);
            last_right_tx = right_tx_bit;
        }*/
        /*
        // Left FPGA TX
        int right_rx_bit = (right->uio_in >> 1) & 1;
        if (right_rx_bit != last_right_rx) {
            printf("time %llu: Right RX = %d\n", sim_time, right_rx_bit);
            last_right_rx = right_rx_bit;
        }*/
